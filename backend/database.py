...
def upsert_campaigns(campaigns_data):
    ...
    for camp in campaigns_data:
        ...
        c.execute('''
            INSERT INTO campaigns (id, title, send_time, data_json, updated_at)
        ...
        ''', (c_id, title, send_time, data_json))
        
    conn.commit()
    conn.close()

def get_cached_campaigns(days=30):
    ...
    # SQLite doesn't have native datetime types, stores as strings usually.
    # Mailchimp sends ISO8601 strings, e.g., "2023-10-27T10:00:00+00:00"
    # String comparison works if format is consistent ISO.
    
    # Simpler approach: fetch all and filter in python or just sort limit
